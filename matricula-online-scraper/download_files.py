...
class Downloader:
    ...
    def parse_image_URLs_and_labels(self, record_reponse_text):
        ...
        # If a specific page number is in the URL, download only that page
        if self.target_page is not None:
            # Page numbers are 1-indexed, so subtract 1 for 0-indexed list
            page_index = self.target_page - 1
            if 0 <= page_index < len(self.image_URLs_and_labels):
                self.image_URLs_and_labels = (self.image_URLs_and_labels[page_index],)
                logging.info(
                    f"Targeting page {self.target_page} from URL - downloading 1 image"
                )
            else:
                logging.warning(
                    f"Page {self.target_page} not found in archive (total pages: {len(self.image_URLs_and_labels)}). "
                    f"Downloading all images."
                )
        # Otherwise, if a range is provided and the range is less than the number of scraped files, use it
        elif self.file_range is not None and self.file_range < len(
